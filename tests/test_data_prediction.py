...
from intern_src.utils_package.data_prediction import validate_inputs, drop_missing_inputs
...
def test_drop_missing_inputs_removes_na(sample_input_data):
    # Injecter une valeur manquante dans une variable non autorisée à contenir des NaN
    var_with_na = [col for col in sample_input_data.columns
                   if col not in (
                               core.CATEGORICAL_FREQUENT_MISSING + core.CATEGORICAL_FREQUENT_CATEGORY + core.NUMERICAL_MISSING_VARS)][
        0]
    ...
    # On s'assure que la ligne avec NaN sur une variable critique a été supprimée
