...
from intern_src.utils_package.data_transformation import TemporalTransformer, logarithm_transformer, Mapper
from tests.testconfig import sample_input_data, sample_continuous_log_data


def test_temporal_transformer_initialization():
    transformer = TemporalTransformer(variables=['var1', 'var2'], reference_variable='ref')
...
def test_temporal_transformer_transform(sample_input_data):
    """
    Teste que la transformation temporelle est correctement appliquée
    en soustrayant chaque variable temporelle de la variable de référence.
    """
    # Assure que toutes les colonnes nécessaires sont là
    required_columns = core.TEMPORAL_VARS + [core.REF_VARS]
    for col in required_columns:
        assert col in sample_input_data.columns, f"{col} manquant dans les données d'entrée"

    transformer = TemporalTransformer(
        variables=core.TEMPORAL_VARS,
        reference_variable=core.REF_VARS
    )
...
def test_logarithm_transformer(sample_continuous_log_data):
    """
    Teste que la transformation logarithmique est correctement appliquée
...
def test_feature_match(sample_input_data):
    """
    Vérifie que les features définies dans core.FEATURES correspondent exactement
    ...
    assert actual_features == expected_features, f"L'ordre des colonnes ne correspond pas : {actual_features} != {expected_features}"