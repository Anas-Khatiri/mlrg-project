...
def upload_file(file: UploadFile = File(...)):
    try:
        ...
        # Check the columns
        missing_cols = [
            col for col in core.FEATURES if col not in data_transformed.columns
        ]
        if missing_cols:
            raise HTTPException(
        ...
        results = make_prediction(
            input_data=data_transformed[core.FEATURES].to_dict(orient="records")
        )
        return {"predictions": results["predictions"]}
    except Exception as e:
        raise HTTPException(
            status_code=400, detail=f"Error of file processing: {str(e)}"
        )
