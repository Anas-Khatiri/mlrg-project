...
def drop_missing_inputs(*, input_data: pd.DataFrame) -> pd.DataFrame:
    ...
    new_vars_with_na = [
        ...
        if var
        not in core.CATEGORICAL_FREQUENT_MISSING
...
def validate_inputs(*, input_data: pd.DataFrame) -> Tuple[pd.DataFrame, Optional[dict]]:
    ...
    input_data.rename(columns=core.VARIABLES_TO_RENAME, inplace=True)
    # input_data = input_data.drop(['Id'], axis=1)
    input_data["MSSubClass"] = input_data["MSSubClass"].astype("O")
    # relevant_data = input_data[input_data.columns].copy()
    relevant_data = input_data.copy()
    validated_data = drop_missing_inputs(input_data=relevant_data)
    errors = None
...
class HouseDataInputSchema(BaseModel):
    ...
    ThreeSsnPorch: Optional[int]  # renamed
