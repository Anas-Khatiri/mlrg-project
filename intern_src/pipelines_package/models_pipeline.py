...
def create_base_pipeline():
    """Creates a pipeline with a specified alpha value for the Lasso model."""
    model_pipeline = Pipeline(
        [
            ...
            (
                "missing_imputation",
                CategoricalImputer(
                    imputation_method="missing", variables=core.CATEGORICAL_FREQUENT_MISSING
                ),
            ),
            (
                "frequent_imputation",
                CategoricalImputer(
                    imputation_method="frequent", variables=core.CATEGORICAL_FREQUENT_CATEGORY
                ),
            ),
            # numerical missing values
            ("missing indicator", AddMissingIndicator(variables=core.NUMERICAL_MISSING_VARS)),
            (
                "mean imputation",
                MeanMedianImputer(imputation_method="mean", variables=core.NUMERICAL_MISSING_VARS),
            ),
            # temporal variables
            (
                "temporal transformer",
                TemporalTransformer(variables=core.TEMPORAL_VARS, reference_variable=core.REF_VARS),
            ),
            ...
            (
                "rare labels",
                RareLabelEncoder(tol=0.01, n_categories=1, variables=core.CATEGORICAL_VARS),
            ),
            (
                "ordinal encoder",
                OrdinalEncoder(encoding_method="ordered", variables=core.CATEGORICAL_VARS),
            ),
        ]
    )
...
def create_lasso_pipeline(alpha=0.001, max_iter=100, tol=1e-2):
    """Crée un pipeline complet avec Lasso et des hyperparamètres configurables."""
